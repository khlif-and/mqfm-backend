...
func (h *PlaylistHandler) Update(c *gin.Context) {
	...
	if existing.UserID != userID {
		resp.Error(c, http.StatusForbidden, constant.MsgUnauthorized, nil)
		return
	}
...
func (h *PlaylistHandler) Search(c *gin.Context) {
	...
	playlists, err := h.service.Search(query)
	if err != nil {
	...
	resp.Success(c, http.StatusOK, constant.MsgPlaylistSearchOK, result)
}
...
func (h *PlaylistHandler) AddAudio(c *gin.Context) {
	...
	existing, err := h.service.GetByID(input.PlaylistID)
	if err != nil {
		resp.Error(c, http.StatusNotFound, constant.MsgPlaylistNotFound, err.Error())
		return
	}
...
func (h *PlaylistHandler) Share(c *gin.Context) {
	id, ok := helper.ParamToUint(c, "id")
	if !ok {
		resp.Error(c, http.StatusBadRequest, constant.MsgInvalidIDFormat, nil)
		return
	...
	token, err := h.service.SharePlaylist(id)
	if err != nil {
		resp.Error(c, http.StatusInternalServerError, constant.MsgPlaylistShareFail, err.Error())
		return
	}

	resp.Success(c, http.StatusOK, constant.MsgPlaylistShareOK, gin.H{"share_token": token})
}
